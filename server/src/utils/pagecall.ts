...
export class PageCall {
    ...
    /**
     * 생성자
     * @param key PCA Application Key
     * @param endpoint PCA API Endpoint
     */
     ...
     */
    async createUser(userId: string, name: string, metadata: Record<string, any> = {}): Promise<User> {
        try {
     ...
     * @param layoutId 레이아웃 ID
     */
    async addMember(roomId: string, userId: string, options?: Record<string, any>, layoutId?: string): Promise<Member> {
        try {
...
export const PCA = new PageCall(env.PAGECALL_KEY, 'https://api.pagecall.net/v1');
